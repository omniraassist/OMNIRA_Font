...
const STYLES = `
  ...
  .twa-step.active .twa-step-dot {
    border-color: var(--em);
    background: rgba(0,229,160,0.12);
    color: var(--em);
  }
  .twa-step.done .twa-step-dot {
    border-color: var(--em);
    background: var(--em);
    color: #00120a;
  }
  ...
  .twa-pipe-step.active .num {
    background: linear-gradient(135deg, var(--em), #93c5fd);
    color: #00120a;
    box-shadow: 0 0 0 4px rgba(0,229,160,0.15);
  }
  .twa-pipe-step .lbl { font-size: 12px; color: var(--soft); white-space: nowrap; }
  .twa-pipe-step.done .lbl,
  .twa-pipe-step.active .lbl { color: var(--text); font-weight: 600; }
  .twa-pipe-line { flex: 1; height: 1px; background: var(--border); min-width: 16px; }
`;

export function PostLoginTwilioAssigning({ onDone }) {
        ...
        <div className="twa-card">
          ...
          <div className="twa-steps">
            {STEPS.map((s, i) => (
              <div
                key={i}
                className={`twa-step${i < currentStep ? ' done' : i === currentStep ? ' active' : ''}`}
              >
                <div className="twa-step-dot">
                  {i < currentStep ? '✓' : i + 1}
                </div>
